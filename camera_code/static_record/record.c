...
/// Records live video from raspberry pi
/// @author: Will Merges
...
char* generate_pipeline(char* fps, char* outfile) {
    ...
    strcpy(pipeline, "gst-launch-1.0 -e v4l2src device=/dev/video0 ! video/x-raw, width=1920, height=1080, target-bitrate=8000000, framerate=");
    strcat(pipeline, fps);
